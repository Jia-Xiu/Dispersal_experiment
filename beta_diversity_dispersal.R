...
#my_palette = c(brewer.pal(7, "Greys")[c(1:7)])
...
ggsave("PCoA_bray_Year_day.jpg", width = 8.5, height = 6.5, units = "cm", device = "jpeg", p0, scale = 1.5, dpi = 300)
...
# ========================================================================================================
# PERMANOVA each question
# ========================================================================================================
datasets <- c("sea", "sea_strl")
years <- c("0", "70")
for (i in datasets) {
  if (i == "sea") {
    subcom <- subset(com_treatments, grepl("sea", row.names(com_treatments))) 
  } else if (i == "sea_strl") {
    subcom <- subset(com_treatments, grepl("_f1_", row.names(com_treatments)) | grepl("_f2_", row.names(com_treatments)))
  }
  subcom <- subcom[ , colSums(subcom) != 0]
  dim(subcom)
  subcom[1:5, 1:2]
  ...
  dist <- as.dist(as.matrix(dist))
  if (i == "seawater") {
    (result_subcom <- adonis(dist ~ Soil*Day*Frequency, data=df, permutation=9999)) }
  else if (i == "sea_strl") {
    (result_subcom <- adonis(dist ~ Soil*Water*Day*Frequency, data=df, permutation=9999)) }
  result_subcom
  write.csv(result_subcom$aov.tab, paste("PERMANOVA_bray", i, "without_day0.csv", sep = "_"))
}

# ========================================================================================================
# PERMANOVA each question in each soil
# ========================================================================================================
...
for (i in datasets) {
  for (year in years) {
    ...
    if (i == "sea") {
      subcom <- subset(subcom, grepl("sea", row.names(subcom))) 
    } else if (i == "sea_strl") {
      subcom <- subset(subcom, grepl("_f1_", row.names(subcom)) | grepl("_f2_", row.names(subcom)))
    }
    subcom <- subcom[ , colSums(subcom) != 0]
    dim(subcom)
    subcom[1:5, 1:2]
    ...
    dist <- as.dist(as.matrix(dist))
    if (i == "sea") {
      (result_subcom <- adonis(dist ~ Day*Frequency, data=df, permutation=9999)) 
      } else if (i == "sea_strl") {
      (result_subcom <- adonis(dist ~ Water*Day*Frequency, data=df, permutation=9999)) 
        }
    result_subcom
    write.csv(result_subcom$aov.tab, paste("PERMANOVA_bray", i, year, "yr_without_day0.csv", sep = "_"))
  }
}
...
p <- list()
freqs <- c("0_f1", "0_f2", "0_f3", "0_f4", "70_f1", "70_f2", "70_f3", "70_f4")

for (fr in freqs) {
  
  cat("analysis for frequency", fr)
  subcom <- subset(com, grepl(paste("^", fr, sep = ""), rownames(com)))
  subcom <- subcom[, !colSums(subcom) == 0]
  dim(subcom)
  subcom[1:5, 1:2]
  ...
  my_palette <- c("#E69F00", "#56B4E9","#F0E442",  "#009E73", "#CC79A7", "#0072B2")
  my_palette = c(brewer.pal(7, "Greys")[c(2:7)])
  
  head(df)
  if (fr == "0_f1") {
    plot_title = "0 years (1x / 7 days)"
  } else if (fr == "0_f2") {
    plot_title = "0 years (1x / 3 days)"
  } else if (fr == "0_f3") {
    plot_title = "0 years (1x / day)"
  } else if (fr == "0_f4") {
    plot_title = "0 years (2x / day)"
  } else if (fr == "70_f1") {
    plot_title = "70 years (1x / 7 days)"
  } else if (fr == "70_f2") {
    plot_title = "70 years (1x / 3 days)"
  } else if (fr == "70_f3") {
    plot_title = "70 years (1x / day)"
  } else if (fr == "70_f4") {
    plot_title = "70 years (2x / day)"
  }
  
  print(plot_title)
  
  ( p[[fr]] <- ggplot(df, aes(x = pc1, y = pc2, shape = Water, color = Day, fill = Day))+
      geom_point(size = 5, alpha = .9)+
      scale_colour_manual(values = my_palette) +
      scale_fill_manual(values = my_palette, guide=guide_legend(override.aes = list(shape=21))) +
...
(p1 <- (ggarrange(p[[1]], p[[2]], p[[3]], p[[4]], p[[5]], p[[6]], p[[7]], p[[8]], 
                  labels = c("A", "B", "C", "D", "E", "F", "G", "H"), 
                  common.legend = TRUE, legend = "right", ncol = 4, nrow = 2)))

ggsave("PCoA_bray_seperate_by_freq_soil_raw.pdf", width = 22, height = 10, 
       units = "cm", p1, scale = 1.5, dpi = 300)
